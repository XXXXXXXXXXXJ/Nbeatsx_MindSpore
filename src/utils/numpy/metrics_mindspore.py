...
from ..pytorch.losses import divide_no_nan
...
def mae(y, y_hat, weights=None):
  """兼容 MindSpore Tensor 和 NumPy 数组"""
  if hasattr(y, 'asnumpy'):  # 如果是 MindSpore Tensor
    y = y.asnumpy()
  if hasattr(y_hat, 'asnumpy'):
    y_hat = y_hat.asnumpy()
  if weights is not None and hasattr(weights, 'asnumpy'):
