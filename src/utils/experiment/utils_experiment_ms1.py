...
from ..numpy.metrics import rmae, mae, mape, smape, rmse
from ..data.utils import Scaler
from ..pytorch.ts_dataset_ms import TimeSeriesDatasetMS
from ..pytorch.ts_loader_ms import TimeSeriesLoaderMS
from nbeats.nbeats_mindspore_main import NbeatsMS
...
def run_val_nbeatsx(hyperparameters, Y_df, X_df, data_augmentation, random_validation, trials, trials_file_name):
    ...
    mc = copy.deepcopy(hyperparameters)
    ...
    include_var_dict = {'y': [],
                        ...
                        'Exogenous2': [],
                        'week_day': []}
    # print(">>> Final mc['include_var_dict'] =", mc['include_var_dict'])
    ...
    ts_dataset = TimeSeriesDatasetMS(Y_df=Y_df_scaled, X_df=X_df_scaled, ts_train_mask=train_outsample_mask)
    # ts_dataset.f_cols = ['week_day']
    # Loaders object. Sample windows of dataset object.
    # For more information on each parameter, refer to comments on Loader object.
    train_ts_loader = TimeSeriesLoaderMS(model='nbeats',
    ...
    val_ts_loader = TimeSeriesLoaderMS(model='nbeats',
                                     ts_dataset=ts_dataset,
                                     window_sampling_limit=mc['window_sampling_limit_multiplier'] * mc['output_size'],
                                     offset=0,
                                     input_size=int(mc['input_size_multiplier'] * mc['output_size']),
    ...
    model = NbeatsMS(input_size_multiplier=mc['input_size_multiplier'],
                   output_size=int(mc['output_size']),
...
def run_test_nbeatsx(mc, Y_df, X_df, len_outsample):
    ...
    for split in range(n_splits):
        ...
        ts_dataset_test = TimeSeriesDatasetMS(Y_df=Y_df_scaled, X_df=X_df_scaled, ts_train_mask=test_mask)
        test_ts_loader = TimeSeriesLoaderMS(model='nbeats',
